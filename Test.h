...
{ result_.countCheck(); \
  if (!(condition)) \
{ result_.addFailure (Failure (this, __LINE__, #condition)); return; } }
...
{ result_.countCheck(); \
  if ((expected) == (actual)) ;\
  else {result_.addFailure(EqualsFailure(this, __LINE__, StringFrom(expected), StringFrom(actual)));\
        return;}\
}
...
{ result_.addFailure(EqualsFailure(this, __LINE__, StringFrom(expected), StringFrom(actual)));\
return;} }
...
{ result_.addFailure (EqualsFailure(this, __LINE__, StringFrom(expectedTemp), \
...
{ result_.addFailure (EqualsFailure(this, __LINE__,\
StringFrom((double)expectedTemp), StringFrom((double)actualTemp))); return; } }


//Fail if you get to this macro
#define FAIL(text) \
{ result_.addFailure (Failure (this, __LINE__, (text))); return; }
